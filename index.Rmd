...
grading_table <- tibble::tribble(
  ...
  dplyr::mutate(
    `Total points` = (`Number assigned` * `Points per assignment`),
    `Share of grade` = `Total points` %>%
      magrittr::divide_by( sum(`Total points`)) %>%
      scales::percent(accuracy = 1L)
  )
kable(grading_table, table.attr = "style='width:99%';") %>%
  kableExtra::column_spec(column = 1:5, border_right = TRUE)
```
