...
Green new hires $G[0,T]$ who have never worked anywhere before
...
* Permanent retirement $R[0,T]$
* Death again $D[0,T]$

Why didn't I list employee termination?<br>
...
Population is defined as employees at the company:

* Terminations who remain in the workforce $S[0,T]$ (out-migration)
* Re-hires $W[0,T]$ (in-migration)
* Hires from other companies $H[0,T]$ (in-migration)

Population is defined as a subset of employees at company:
...
From this data, we can easily compute number of terminations:
...
* Therefore, if growth rate isn't constant, the pattern of growth during the period affects the number of person-years lived!
* This becomes important to consider once we start approximating person-years from period counts
...
If period length is small relative to the pace at which growth rates change (i.e., relative to rate's acceleration)
</details>
