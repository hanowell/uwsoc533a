...
$T_x = 0$ for the final age group in this case because we know everyone in the cohort died before age 80.
...
<details>
<summary>How would this assumption perform death count were increasing exponentally over the period? **Tap for answer**</summary>
...
Here's the data (changing the `Age` column to `x` to match our notation):
...
ukr_plt_1x1_2013 <- ukr_plt_1x1_2013 %>%
  dplyr::mutate(
    Tx = rev(Lx) %>% # Reverse person-years column so sum back to x = 0
      ...
      rev() %>% # Reverse the cumulative sum back in age order
      # Tx = Lx for final age group since we don't know maximal age at death
      dplyr::coalesce(Lx)
  )
```
...
The difference between two life expectancies at birth $e^o_0(2) - e_0^0(1)$ can be decomposed into separate contributions ${}_{n}\Delta_x$ from each age group:

$$
e^o_0(2) - e_0^0(1) = \sum_0^\infty {}_{n}\Delta_x
$$
