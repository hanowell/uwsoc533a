...
Remember to fill in the open-ended interval ${}_{\infty}d_{x^*} = l_{x^*}$


```r
ukr_plt_1x1_2013 <- ukr_plt_1x1_2013 %>%
  dplyr::mutate(dx = dplyr::coalesce(lx * qx, lx))
```
...
Once a single cohort has lived through all groups, these conditions result in:
