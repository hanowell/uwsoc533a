...
```{r simple_sim, echo=FALSE, message=FALSE, warning=FALSE}
b2022 <- 1000 # Initial births
r <- 0.05 # Assumed annualized growth rate in number of births
cohorts <- tibble::tibble(
  cohort_date = seq(as.Date("2022-01-01"), as.Date("2028-01-01"), "year"),
  cohort_index = 0:6,
  cohort_color = c(
    "black", "red", "orange", "goldenrod", "green", "blue", "purple"
  )
)
clt <- tibble::tibble(
  a = 0:5,
  xp0 = c(1.00, 0.60, 0.40, 0.20, 0.05, 0.00),
  lx = b2022 * xp0
)
simulation <- cohorts$cohort_date %>%
  purrr::map(function(x) {tibble::tibble(
    cohort_date = x,
    cohort_index = cohorts$cohort_index[cohorts$cohort_date == x],
    cohort_color = cohorts$cohort_color[cohorts$cohort_date == x],
    period = c(cohort_date, cohort_date %m+% lubridate::years(1:5)),
    Age = 0:5
  )}) %>%
  dplyr::bind_rows() %>%
  dplyr::mutate(
    size_numeric = b2022 * clt$xp0 * exp(r * cohort_index),
    size_equation = dplyr::case_when(
      size_numeric == 0 ~ paste0(
        "$\\color{", cohort_color, "}{", size_numeric, "}$"),
      cohort_index == 0 ~ paste0("$", size_numeric, "$"),
      cohort_index == 1 ~ paste0(
        "$\\color{", cohort_color, "}",
        "{", b2022 * clt$xp0, " \\cdot ", "e^r{\\text{ }}}",
        "$"
      ),
      cohort_index > 1 ~ paste0(
        "$\\color{", cohort_color, "}",
        "{", b2022 * clt$xp0, " \\cdot ", "e^{", cohort_index, "r}}",
        "$"
      )
    )
  )
...
```{r simple_sim_clt_plot, echo=FALSE, message=FALSE, warning=FALSE}
clt %>%
  dplyr::select(a, xp0) %>%
  setNames(nm = c("Exact age $a$", "$l_a/l_0 = p(a)$")) %>%
...
```{r simple_sim_cohorts_table, echo=FALSE, message=FALSE, warning=FALSE}
simulation %>%
  dplyr::filter(period <= "2028-01-01") %>%
  dplyr::select(Age, period, size_equation) %>%
  tidyr::pivot_wider(id_cols = "Age",
                     names_from = "period",
                     values_from = "size_equation") %>%
  knitr::kable(align = "r") %>%
  kableExtra::kable_classic(full_width = FALSE)
```
...
```{r simple_sim_cohorts_plot1, echo=FALSE, message=FALSE, warning=FALSE}
simulation %>%
  dplyr::filter(Age < 5) %>%
  ggplot2::ggplot() +
  ggplot2::aes(x = period, y = size_numeric, color = factor(cohort_date)) +
  ggplot2::geom_point() +
  ggplot2::geom_line() +
  ggplot2::geom_line(aes(group = Age), linetype = "dashed", color = "black") +
  ggplot2::annotate(
    "text", x = as.Date("2022-05-01"), y = 1100, label = "Age = 0"
  ) +
  ggplot2::annotate(
    "text", x = as.Date("2022-05-01"), y = 600, label = "Age = 1"
  ) +
  ggplot2::annotate(
    "text", x = as.Date("2023-05-01"), y = 400, label = "Age = 2"
  ) +
  ggplot2::annotate(
    "text", x = as.Date("2024-05-01"), y = 200, label = "Age = 3"
  ) +
  ggplot2::annotate(
    "text", x = as.Date("2025-04-01"), y = 60, label = "Age = 4"
  ) +
  ggplot2::scale_color_manual(values = c(
    "black", "red", "orange", "#ABA300", "dark green", "blue", "purple"
  )) +
  ggplot2::labs(x = "Period", y = "Cohort size") +
...
```{r simple_sim_cohorts_plot2, echo=FALSE, message=FALSE, warning=FALSE}
age_comps <- simulation %>%
  dplyr::filter(period <= "2028-01-01") %>%
  dplyr::with_groups(c(period, Age), summarize, size =size_numeric) %>%
  dplyr::group_by(period) %>%
  dplyr::mutate(cx = size / sum(size)) %>%
  dplyr::ungroup()
cc <- scales::seq_gradient_pal("dark blue", "grey", "Lab")(seq(0,5,length.out=20))
age_comps %>%
  ggplot2::ggplot() +
  ggplot2::aes(x = period, y = cx, fill = factor(Age)) +
  ggplot2::geom_area(position = "stack") +
  ggplot2::geom_hline(
    yintercept = c(
      1 - cumsum(age_comps$cx[age_comps$period == max(age_comps$period)]), 1
    ),
    color = "light gray"
  ) +
  ggplot2::annotate(
    "text", x = as.Date("2027-01-01"), y = 0.534 + (1.000 - 0.534)/2,
    label = "Age = 0", color = "white"
  ) +
  ggplot2::annotate(
    "text", x = as.Date("2027-01-01"), y = 0.268 + (0.534 - 0.268)/2,
    label = "Age = 1", color = "white"
  ) +
  ggplot2::annotate(
    "text", x = as.Date("2027-01-01"), y = 0.099 + (0.268 - 0.099)/2,
    label = "Age = 2", color = "white"
  ) +
  ggplot2::annotate(
    "text", x = as.Date("2027-01-01"), y = 0.019 + (0.099 - 0.019)/2,
    label = "Age = 3", color = "white"
  ) +
  ...
  ggplot2::labs(x = "Period", y = "Age composition") +
  ggplot2::theme_minimal() +
...
```{r chile1992, message=FALSE, warning=FALSE}
# Female life table
...
```{r momentum, echo=TRUE, warning=FALSE, message=FALSE}
