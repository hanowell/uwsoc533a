...
cohort_lt <- age_groups %>%
  ...
  dplyr::summarize(
    dx = sum(!is.na(exit_age)),
    Ax = sum(indiv_A),
    ax = mean(indiv_A)
  ) %>%
  dplyr::ungroup() %>%
  dplyr::mutate(
    lx = nrow(cohort_data) - cumsum(dplyr::lag(dx, default = 0)),
    qx = dx / lx,
    px = 1 - qx,
    Lx = (lx - dx) * n + ifelse(is.na(Ax), 0, Ax),
    mx = dx / Lx,
    Tx = rev(Lx) %>% dplyr::coalesce(0) %>% cumsum() %>% rev(),
    ex = Tx / lx
  )
cohort_lt_xL <- cohort_lt %>% dplyr::select(x, Lx)
```
...
cohort_lt_xL %>%
  dplyr::mutate(Lx = dplyr::coalesce(Lx, 0)) %>%
  knitr::kable() %>%
...
For example, for newborns, we have ${}_{1}m_0$ = `r cohort_lt[["dx"]][cohort_lt[["x"]] == 0]`/`r cohort_lt[["Lx"]][cohort_lt[["x"]] == 0]` $\approx$ `r round(cohort_lt[["mx"]][cohort_lt[["x"]] == 0], 2)`
</details>
<br><br>
...
cohort_lt_display <- cohort_lt %>%
  dplyr::mutate(Lx = dplyr::coalesce(Lx, 0)) %>%
  dplyr::select(x, n, xpn, lx, dx, qx, px, Lx, Tx, ex, mx, ax) %>%
  setNames(
    nm = c(
      "$x$",
      "$n$",
...
ukr_plt_1x1_2013 %>%
  ...
  ggplot2::xlab(latex2exp::TeX("$x$", italic = TRUE)) +
  ggplot2::ylab(latex2exp::TeX("${}_{n}d_x$", italic = TRUE)) +
  ggplot2::theme_minimal()
```
...
```{r ex_diff_decomp, echo=FALSE, message=FALSE, warning=FALSE}
tibble::tibble(
