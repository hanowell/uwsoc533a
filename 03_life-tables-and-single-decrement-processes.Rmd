...
ukr_mx_1x1_2013 <- HMDHFDplus::readHMDweb(
  ...
  setNames(nm = c("x", "nmx")) %>%
  dplyr::mutate(nx = dplyr::lead(x) - x)
```
...
```{r ukr_mx_display, echo=FALSE, warning=FALSE, message=FALSE}
ukr_mx_1x1_2013 %>%
  knitr::kable() %>%
  kableExtra::kable_paper("hover", full_width = FALSE, position = "left") %>%
  kableExtra::column_spec(2:ncol(ukr_mx_1x1_2013), border_left = TRUE)
```
...
```{r ukr_nqx, warning=FALSE, message=FALSE}
...
```{r ukr_npx, warning=FALSE, message=FALSE}
ukr_plt_1x1_2013 <- ukr_plt_1x1_2013 %>%
...
```{r ukr_ndx, warning=FALSE, message=FALSE}
ukr_plt_1x1_2013 <- ukr_plt_1x1_2013 %>%
...
```{r ukr_e0x, warning=FALSE, message=FALSE}
ukr_plt_1x1_2013 <- ukr_plt_1x1_2013 %>%
...
ukr_plt_1x1_2013 %>%
  dplyr::select(x, nmx, nax, nqx, npx, lx, ndx, nLx, Tx, e0x) %>%
  knitr::kable() %>%
  kableExtra::kable_paper("hover", full_width = FALSE, position = "left") %>%
  kableExtra::column_spec(2:10, border_left = TRUE)
```
...
HMDHFDplus::readHMDweb(
  ...
  dplyr::filter(Year == 2013) %>%
  dplyr::select(Age, qx, Lx, ex) %>%
  knitr::kable() %>%
  kableExtra::kable_paper("hover", full_width = FALSE, position = "left") %>%
  kableExtra::column_spec(2:4, border_left = TRUE)
```
...
```{r ukr_lt_plot, echo=FALSE, warning=FALSE, message=FALSE}
ukr_plt_1x1_2013 %>%
...
```{r ukr_ndx_plot, echo=FALSE, warning=FALSE, message=FALSE}
ukr_plt_1x1_2013 %>%
  ggplot2::ggplot(aes(x = x, y = ndx)) +
  ggplot2::geom_line() +
  ggplot2::labs(title = "Number dying by age group") +
  ggplot2::xlab(latex2exp::TeX("$x$", italic = TRUE)) +
